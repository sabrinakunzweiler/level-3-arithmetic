...
class AbelianSurfaceHessianForm(AlgebraicScheme_subscheme_projective):
    ...
    def jacobian(self):
        # returns a jacobian associated to the Hessian form
        # together with the symplectic basis P1, P2, Q1, Q2
        # using the formulas from Nasserden's thesis, appendix
        
        R = PolynomialRing(self._base_ring, 'X')
        X = R.gen(0)
        a0, a1, a2, a3, a4 = self._h
        assert a0 == 1
        ...
        FF = [ G**2 + 4*λ*H**3 for (H, λ, G) in HLGs ]
        assert len(Set(FF)) == 1

        C = HyperellipticCurve(FF[0])
        J = C.jacobian()

        P1, P2, Q1, Q2 = [ J(H, G) for (H, λ, G) in HLGs ]
        B = (P1, P2, Q1, Q2)
        for P in B:
            assert 3*P == J(0)
    ...
    def jacobian_old(self):
        # TODO: clean up? might be useful if the new method fails
