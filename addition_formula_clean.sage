## trying to mess around with the form to clean it up
## right now, the addition computes the negation!
## so add(P, Q) == -(P+Q)
...
add_M = Matrix(R, [
    [ 0,     0,      0,      0,      0],
    [ 0,     0, -t1*t2, -t1*t3, -t1*t4],
    [ 0, t2*t1,      0,  t2*t3, -t2*t4],
    [ 0, t3*t1, -t3*t2,      0,  t3*t4],
    [ 0, t4*t1,  t4*t2, -t4*t3,      0]
])
...
def add(P, Q):
    ...
    _,t1,_,t2,t3,t4,_,_,_ = odd_two_torsion_point1
    ...
    # negation because of reshaped vector_form
    return -B(result)
...
def dbl(P):
    ...
    vec1 = vector_form(P)
    result = []
    sig = [1, 2, 0, 4, 5, 3, 7, 8, 6]
    for i in range(9):
        result.append(vec1[sig[i]] * mat * vec1[i])

    # negation because of reshaped vector_form
    return -B([result[sig[i]] for i in range(9)])

# print(add(P, -P, B, odd_two_torsion_point1))
# print(add(Q, Q, B, odd_two_torsion_point1))

print(f"addition works: {add(P, Q) == PQ}")
print(f"doubling works: {dbl(P) == 2*P + odd_two_torsion_point1}")
