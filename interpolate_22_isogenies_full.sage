...
# First guess for the required degrees
...
for s in range(number_of_monomials + 3):
    rows = get_rows(*get_meta_sample())
    mat += rows

M = Matrix(Fp, mat)

print(f'Dimensions of interpolation matrix: {M.dimensions()}')

K = M.right_kernel()

bas = K.basis()

print(f'Dimension of kernel: {len(bas)}')
